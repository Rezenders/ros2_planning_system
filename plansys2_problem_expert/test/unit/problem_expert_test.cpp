...
TEST(problem_expert, exist_predicate)
{
  ...
  ASSERT_TRUE(
    problem_expert.existPredicate(
      parser::pddl::fromStringPredicate(
        "(inferred-not_same_person jack alice)")));
  ASSERT_FALSE(
    problem_expert.existPredicate(
      parser::pddl::fromStringPredicate(
        "(inferred-not_same_person jack jack)")));
  ASSERT_TRUE(
    problem_expert.existPredicate(
      parser::pddl::fromStringPredicate(
        "(person_at jack bedroom)")));
...
TEST(problem_expert, get_predicate_with_derived)
{
  ...
  ASSERT_TRUE(
    std::find(
      predicates_names.begin(), predicates_names.end(), "(inferred-not_same_person jack alice)") !=
    predicates_names.end());
  ASSERT_FALSE(
    std::find(
      predicates_names.begin(), predicates_names.end(), "(inferred-not_same_person jack jack)") !=
    predicates_names.end());
  ASSERT_TRUE(
    std::find(
